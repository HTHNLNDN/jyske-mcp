"""
Covers the NULL-transaction_id dedup fix in lib/storage.py.
...
deterministic synthetic id (lib.storage.synthetic_transaction_id) so the
...
longer creates tables itself (see lib/storage.py's _check_schema_version),
